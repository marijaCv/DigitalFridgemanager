...
public class DatabaseHelper extends SQLiteOpenHelper implements DatabaseHelperInterface {
    ...
    //static DatabaseHelper dbh = new DatabaseHelper();
    ...
    //public static DatabaseHelper getInstance(Context applicationContext){
    //    return dbh;
    //}
    @Override
...
//        }
    ...
    public void dry(int ID, String value, String columnItem){
        String query = "UPDATE " + TABLE_NAME +  " SET " + columnItem + " = '" + value +
                "' WHERE " + COLUMN_ID + " = " + ID;
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL(query);

    }
    // ????????????? DRY, VIDI I ZA SOLID TACNIJE ZA "D" : selectedActivity.class
